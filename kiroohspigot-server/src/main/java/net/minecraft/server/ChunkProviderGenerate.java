...
public class ChunkProviderGenerate implements IChunkProvider {
    ...
    public Chunk getOrCreateChunk(int i, int j) {
        ...
        if (this.n.paperSpigotConfig.generateCaves && this.n.generatorConfig.cavesMultiplier > 0) {
            if (Math.abs(x) > 100 || Math.abs(z) > 100) {
                this.t.a(this, this.n, i, j, ablock);
            }
        }
        if (this.n.paperSpigotConfig.generateCanyon) {
    ...
    public void getChunkAt(IChunkProvider ichunkprovider, int i, int j) {
        ...
        if (biomebase != BiomeBase.DESERT && biomebase != BiomeBase.DESERT_HILLS && !flag && this.i.nextInt(4) == 0) {
            ...
            if(Math.abs(k1) > 100 || Math.abs(i2) > 100) {
