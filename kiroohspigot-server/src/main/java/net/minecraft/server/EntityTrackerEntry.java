...
public class EntityTrackerEntry {
    ...
    private boolean x;
    public boolean n;
    public Set trackedPlayers = new LinkedHashSet(); // GOMID - LHS has faster iteration
    ...
    public EntityTrackerEntry(EntityTracker entityTracker, Entity entity, int i, int j, boolean flag) { // GOMID
        this.entityTracker = entityTracker; // GOMID
        this.tracker = entity;
        this.b = i;
        this.c = j;
        this.u = flag;
        ...
        // GOMID start
        if (SLSpigotConfig.disableTracking) {
    ...
    private final Consumer<EntityPlayer> addNearPlayersConsumer = new Consumer<EntityPlayer>() {
        ...
        public void accept(EntityPlayer entityPlayer) {
            if (!SLSpigotConfig.disableTracking || tracker.passenger == entityPlayer) updatePlayer(entityPlayer);
        }
    };
    // GOMID end
        ...
        if (this.tracker instanceof EntityLiving) {
            ...
            if (!set.isEmpty()) {
                ...
                if (this.tracker instanceof EntityPlayer) {
                    ...
                    ((EntityPlayer) this.tracker).playerConnection.sendPacket(new PacketPlayOutUpdateAttributes(this.tracker.getId(), set)); // GOMID
                }
                // CraftBukkit end
    ...
    public void updatePlayer(EntityPlayer entityplayer) {
        // org.spigotmc.AsyncCatcher.catchOp( "player tracker update"); // Spigot // GOMID
        if (entityplayer != this.tracker) {
            // GOMID start - this.tracker.locN / 32 -> this.tracker.locN
            double d0 = entityplayer.locX - this.tracker.locX;
            double d1 = entityplayer.locZ - this.tracker.locZ;
            // GOMID end
            int range = this.getRange();
            this.b = TrackingRange.getEntityTrackingRange(this.tracker, 0);
            ...
            if (d0 >= (double) (-range) && d0 <= (double) range && d1 >= (double) (-range) && d1 <= (double) range) {
                if (!this.trackedPlayers.contains(entityplayer) && (this.d(entityplayer) || this.tracker.attachedToPlayer)) {
                    if (this.tracker instanceof EntityPlayer && withinNoTrack()) return; // GOMID
                    ...
                    // GOMID start - tablist stuff
                    boolean isTarget18 = entityplayer.playerConnection.networkManager.getVersion() > 28; // GOMID
                    ...
                    if (isTarget18) {
                        ...
                        entityplayer.playerConnection.sendPacket(packet);
                        if (trackerInstanceOf && SLSpigotConfig.onlyCustomTab) entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.removePlayer((EntityPlayer) tracker));
                    } else {
                        if (tracker instanceof EntityPlayer && !SLSpigotConfig.onlyCustomTab) {
                            entityplayer.playerConnection.sendPacket(PacketPlayOutPlayerInfo.addPlayer((EntityPlayer) tracker));
                        }
                    ...
                    // GOMID start - fix head rotation packet spam (properly)
                    if (this.tracker instanceof EntityLiving) {
    ...
    public void clear(EntityPlayer entityplayer) {
        ...
        if (this.trackedPlayers.remove(entityplayer)) { // GOMID
            entityplayer.d(this.tracker);
        }
    }
