...
import network.secondlife.spigot.interceptors.MoveHandler;
import network.secondlife.spigot.interceptors.MoveInterceptor;
import network.secondlife.spigot.interceptors.PacketHandler;
import network.secondlife.spigot.interceptors.PacketInterceptor;
...
public class PlayerConnection implements PacketPlayInListener {
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (!this.player.viewingCredits) {
            ...
            if(delta > 0.00390625D && this.checkMovement && !this.player.dead) {
                ...
                if(SLSpigotConfig.usePlayerMoveEvent) {
                ...
                // GOMID start
                if(packetplayinflying.hasPos && delta > 0.0D) {
                    try {
                        SpigotTimings.updatePosition.startTiming();

                        for(MoveInterceptor interceptor : MoveHandler.getListeners()) {
                            interceptor.onUpdateLocation(player, from, to, packetplayinflying);
                        }

                        SpigotTimings.updatePosition.stopTiming();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
                ...
                if(packetplayinflying.hasLook) {
                    try {
                        SpigotTimings.updateRotation.startTiming();

                        for(MoveInterceptor interceptor : MoveHandler.getListeners()) {
                            interceptor.onUpdateRotation(player, from, to, packetplayinflying);
                        }

                        SpigotTimings.updateRotation.stopTiming();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }
        ...
        if (packetplayinblockdig.g() == 4) {
            ...
            if (packetplayinblockdig.g() == 0) {
                if (!this.minecraftServer.a(worldserver, i, j, k, this.player)) {
                    ...
                    if(SLSpigotConfig.fireLeftClickBlock) {
    ...
    public void sendPacket(Packet packet) {
        ...
        try {
            ...
            try {
                for (PacketInterceptor listeners : PacketHandler.getListeners()) {
                    listeners.onPacketSend(this, packet);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public void a(PacketPlayInArmAnimation packetplayinarmanimation) {
        ...
        if (packetplayinarmanimation.d() == 1) {
            ...
            if (SLSpigotConfig.fireLeftClickAir && this.player.isSneaking()) {
                float f = 1.0F;
