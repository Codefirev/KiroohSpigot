                ...
                if (movingobjectposition.entity != null) {
                    if (owner != null) {
                        if (movingobjectposition.entity instanceof EntityPlayer) {
                            ...
                            if (((CraftPlayer) owner.getBukkitEntity()).canSee(hit.getBukkitEntity())) {
                                if (movingobjectposition.entity.damageEntity(DamageSource.projectile(this, this.owner), 0.0F)) {
                                    this.hooked = movingobjectposition.entity;

                                    KnockbackProfile kb = hit.getKnockback();

                                    double velX, velY, velZ;

                                    Vector v = new Vector(motX, motY, motZ).normalize();
                                    velX = (v.getX() / 1.6) * kb.rodH.value;
                                    velY = 0.36 * kb.rodV.value;
                                    velZ = (v.getZ() / 1.6) * kb.rodH.value;
