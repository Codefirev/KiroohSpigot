...
public abstract class BlockLeaves extends BlockTransparent {
    ...
    private void e(World world, int i, int j, int k) {
        // GOMID - Add disableLeavesDecay check.
        if(SLSpigotConfig.disableLeavesDecay) {
            return;
        }

        // SecoondLife start - UHC optimization
        if(SLSpigotConfig.uhc) {
            org.bukkit.block.Block block = world.getWorld().getBlockAt(i, j, k);

            if(SLSpigotConfig.getRandomValue() <= SLSpigotConfig.appleRate) {
                block.getWorld().dropItemNaturally(block.getLocation(),
                        new org.bukkit.inventory.ItemStack(org.bukkit.Material.APPLE));
            }

            if(SLSpigotConfig.luckyLeavesRate != -1 && SLSpigotConfig.getRandomValue() <= SLSpigotConfig.luckyLeavesRate) {
                block.getWorld().dropItemNaturally(block.getLocation(),
                        new org.bukkit.inventory.ItemStack(org.bukkit.Material.GOLDEN_APPLE));
            }
    ...
    public void dropNaturally(World world, int i, int j, int k, int l, float f, int i1) {
        if (!world.isStatic) {
            ...
            if (!SLSpigotConfig.uhc && world.random.nextInt(j1) == 0) {
