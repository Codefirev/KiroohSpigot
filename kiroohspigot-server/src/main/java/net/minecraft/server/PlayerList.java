...
public abstract class PlayerList {
    ...
    protected void b(EntityPlayer entityplayer) {
        if (SLSpigotConfig.disableSaving) return; // MineHQ
        if (SLSpigotConfig.disablePlayerFileSaving) { return; } // Poweruser
    ...
    public void c(EntityPlayer entityplayer) {
        ...
        if (SLSpigotConfig.onlyCustomTab) return; // MineHQ
    ...
    public String disconnect(EntityPlayer entityplayer) { // CraftBukkit - return string
        ...
        for (int i = 0; i < this.players.size(); ++i) {
            ...
            if (entityplayer1.getBukkitEntity().canSee(entityplayer.getBukkitEntity())) {
                if (!SLSpigotConfig.playerListPackets) continue; // MineHQ
                entityplayer1.playerConnection.sendPacket(packet);
            } else {
    ...
    public void tick() {
        ...
        try
        {
            if ( !players.isEmpty() && SLSpigotConfig.updatePingOnTablist)
            {
    ...
    public boolean isWhitelisted(GameProfile gameprofile) {
        return !this.hasWhitelist || this.ops.contains(gameprofile) || this.whitelist.d(gameprofile);
    }

    public boolean isOp(GameProfile gameprofile) {
        return this.ops.contains(gameprofile) || (this.server.N() && this.server.worlds.get(0).getWorldData().allowCommands() && this.server.M().equalsIgnoreCase(gameprofile.getName())) || this.s;
    }
    ...
    public List<GameProfile> getOPs() {
        return this.ops;
    }
