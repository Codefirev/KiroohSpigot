package network.secondlife.spigot.timings;
...
public class ExtendedCustomTimingsHandler extends CustomTimingsHandler {
    ...
    private static File path = new File("logs/LagSpikeLog");
    ...
    public static void tick() {
        if(Bukkit.getPluginManager().useTimings() && SLSpigotConfig.lagSpikeLoggerEnabled) {
            for(CustomTimingsHandler handler: HANDLERS) {
                if(handler.getCurrentTickTotal() > SLSpigotConfig.lagSpikeLoggerTickLimitNanos) {
