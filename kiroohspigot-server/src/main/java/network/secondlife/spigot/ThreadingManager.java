...
import network.secondlife.spigot.pathsearch.PathSearchThrottlerThread;
...
public class ThreadingManager {
    ...
    private ScheduledExecutorService timerService = Executors.newScheduledThreadPool(1, new NamePriorityThreadFactory(Thread.NORM_PRIORITY + 2, "GSpigot_TimerService"));
    ...
    public ThreadingManager() {
        ...
        this.cachedThreadPoolFactory = new NamePriorityThreadFactory(Thread.currentThread().getPriority() - 1, "GSpigot_Async-Executor").setLogThreads(true).setDaemon(true);
    ...
    public void shutdown() {
        ...
        if(!this.cachedThreadPool.isTerminated()) {
            ...
            if(SLSpigotConfig.logRemainingAsyncThreadsDuringShutdown && this.cachedThreadPoolFactory.getActiveCount() > 0) {
